...
func absPathFromPwd(importPath string) (string, bool) {
	pwd, _ := os.Getwd()

	pwdImport := filepath.Join(getParentDirectory(pwd), importPath)
	if fileExists(pwdImport) {
		return pwdImport, true
	}
	return "", false
}
